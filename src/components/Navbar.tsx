...
import { Moon, Sun, Globe, Menu, X, LogOut, User, UserPlus, LogIn } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 w-full py-4 px-[5%] z-[1000] transition-all duration-400 backdrop-blur-xl border-b border-border ${scrolled ? 'py-3 bg-background/95 shadow-lg shadow-primary/5' : 'bg-background/85'}`}>
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          {/* Logo with 3D effect */}
          <Link to="/" className="flex items-center gap-3 cursor-pointer group perspective-500">
            <div className="relative transform-gpu transition-transform duration-500 group-hover:rotate-y-12">
              <img src={logo} alt="CodeMarket" className="w-10 h-10 object-contain transition-all duration-500 group-hover:scale-110" />
              <div className="absolute inset-0 rounded-full bg-primary/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            </div>
            <span className="text-2xl font-extrabold gradient-text animate-glow hidden sm:inline">CodeMarket</span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex gap-3 items-center">
              {!loading && isAuthenticated ? (
                ...
                <>
                  <button onClick={() => setAuthModal('signin')} className="auth-btn flex items-center gap-2 group">
                    <LogIn size={16} className="group-hover:translate-x-0.5 transition-transform" />
                    {t('sign_in')}
                  </button>
                  <button onClick={() => setAuthModal('signup')} className="auth-btn auth-btn-primary flex items-center gap-2 group">
            ...
            {!loading && (
              <div className="flex gap-3 mt-6">
                {isAuthenticated ? (
                  ...
                  <>
                    <button onClick={() => { setAuthModal('signin'); setMobileMenuOpen(false); }} className="auth-btn flex-1 flex items-center justify-center gap-2">
                      <LogIn size={16} />
                      {t('sign_in')}
                    </button>
                    <button onClick={() => { setAuthModal('signup'); setMobileMenuOpen(false); }} className="auth-btn auth-btn-primary flex-1 flex items-center justify-center gap-2">
                      <UserPlus size={16} />
                      {t('sign_up')}
                    </button>
                  </>
